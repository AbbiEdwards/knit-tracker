...
- **Overview** — a progress bar per active project, coloured by the project's own yarn colour, with a separate urgency badge for the deadline. Projects marked `finished` drop off this view.
- **Project details** — pick a project to view and edit everything about it (designer, yarn weight, needle size, gauge, size, dates, status, the Ravelry-page flag, notes), add a brand new project, and see a segmented bar showing each of its stages coloured in by how complete it is.
...
- **Manage stages** — add a project's construction stages and edit any stage's details later. Stages belonging to a `finished` project are hidden here.
- **Analytics** — total hours per project and hours per logged session by stage/yarn weight, built from your own history. Unlike the other tabs, this always includes every project, `finished` ones included.
